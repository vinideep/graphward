...
1. **Refresh the graph** for changed files: `npx gw map . --update --files <a,b,c>` — preserves stable node IDs, rebuilds only what changed.
2. **Re-check evidence** the knowledge base already committed to: `npx gw claims verify --json` — derived claims are re-computed (`verified` / `refuted`), asserted claims are hash-checked (`unverified` / `stale` / `missing`). Refuted, stale and missing claims are your precise worklist. After code changes, re-run `npx gw claims derive .` so the derived baseline matches reality.
