...
Use `context-budget-optimizer` before loading broad intelligence. Do not read all of `.graphward/knowledge-base/` or all graph JSON by default. Build `.graphward/context/context-manifest.md`, then load only relevant slices:
...
Before any code edit, write `.graphward/reports/IMP-XXX-<summary>.md`:
...
- Update `.graphward/aidlc/execution-plan.md` and `.graphward/aidlc/aidlc-state.md`
