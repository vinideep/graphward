...
import { scanContent } from "../manifest/secrets-scan.js";

async function writeTarget(root: string, relativePath: string, content: string): Promise<void> {
  if (relativePath.includes(".graphward/") || relativePath.startsWith(".graphward/")) {
    const scan = scanContent(content, relativePath);
    if (!scan.clean) {
      throw new Error(`Secret detected in ${relativePath}: ${scan.findings[0].pattern}`);
    }
  }
...
export async function install(
  ...
  if (!options.dryRun) {
    ...
    // Default gitignore for content-derived artifacts
    const gitignoreContent = `.hooks-state/
memory/users/
telemetry/
providers/
.verify/
...
reports/
`;
    await writeTarget(root, ".graphward/.gitignore", gitignoreContent);

    if (options.share) {
      const summaryContent = `# GraphWard Architecture Summary
This is a redacted structural summary safe for version control.
(Generated by --share flag)
`;
      await writeTarget(root, ".graphward/architecture-summary.md", summaryContent);
    }
